...
class Window(object):
    def __init__(self):
        ...
        self.root.title("THIS IS A ROOT GAME")
        try:
        ...
        # This is for creating the menu which makes us read all the modules needed.
        mainMenu = Menu(self.root)
        self.root.config(menu = mainMenu)

        subMenu = Menu(mainMenu, tearoff = 0)
        mainMenu.add_cascade(label = "The-Rules", command = self.gameRules)
        mainMenu.add_cascade(label = "You need Help", menu = subMenu)

        subMenu.add_command(label = "The known bugs", command = self.knownBugs)
        subMenu.add_command(label = "The report of bugs", command = self.reportBug)
        subMenu.add_separator()
        subMenu.add_command(label = "Do you want to know how to Play?", command = self.howToPlay)

        self.root.resizable(False,False)
        self.root['bg'] = var.BACKGROUND

        #this is the rules screen for the game... hopes you will enjoy.

        self.gameRulesScreen()

        # This is the pre-loaded FRAMES for this game
        self.frameMenuBar = Frame(self.root, bg = "blue")

        self.frameGame = Frame(self.root)
            #canvas for the game
        self.gameScreen = Canvas(self.frameGame,width=900,height=800,bg="green")
        ...
        Each for player, and this is the last crad played.
        """
        self.field = ["0","0"]
